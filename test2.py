...
def page_kanban():
    ...
    # Tickets holen mit Filtern
    tickets = fetch_tickets(
        archived=show_arch,
        search_term=search if search else None,
        category=filter_cat if filter_cat != "Alle" else None,
        priority=filter_prio if filter_prio != "Alle" else None
    )
...
def main():
    ...
    st.sidebar.markdown(f"Angemeldet als: **{st.session_state.username}**")
    st.sidebar.divider()
    ...
    if st.session_state.role == "admin":
        pages.update({
            "🔧 Admin": page_admin,
            "🗄️ Datenbank": page_database,
        })

    pages["👤 Profil"] = page_profile
